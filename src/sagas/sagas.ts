...
export function* searchCitySaga(
  action: GetSearchOptionsRequestAction
): Generator<CallEffect<string[]> | PutEffect<WeatherAction>, void, string[]> {
  try {
    yield delay(500);
...
export function* weatherSaga(
  action: GetCurrentWeatherAction
):
  | Generator<
  ...
  | SelectEffect {
  try {
    const chosenWeatherApi = yield select(
      (state: State): string => state.weatherReducer.chosenWeatherApi
    );
...
export function* calendarEventsSaga(
  action: GetCalendarEventsAction
): Generator<
