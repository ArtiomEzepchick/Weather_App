...
const CitySearch: React.FC<Props> = ({
  ...
  const handleInputFocus = useCallback((): void => {
    ...
    if (inputRef.current && dataLength === 10) {
      inputRef.current.blur();
      dispatch(
        setError(
          "You've exceeded the max number of saved cities (10). Delete any city to add new ones"
        )
      );
      dispatch(setIsModalOpen(true));
      return;
    }
  }, [dataLength, dispatch, inputRef]);
  ...
  }, [dispatch, inputRef, inputCityValue]);
  ...
  return (
    <form className="city-search-container" onSubmit={handleSubmit}>
      <Space.Compact style={{ width: "100%" }}>
        ...
        />
        <Button disabled={isLoading || !inputCityValue} htmlType="submit">
          Find
        </Button>
      </Space.Compact>
