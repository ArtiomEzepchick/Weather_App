...
const Menu: React.FC<Props> = ({
  ...
  const handleDeleteBtnClick = useCallback((): void => {
    ...
    const newWeatherData: WeatherTransformedData[] =
      allCitiesWeatherData.filter(
        (_, index: number) => index.toString() !== currentMenyKeyRef
      );
    ...
    if (+currentMenyKeyRef >= newWeatherData.length - 1) {
      ...
      localStorage.setItem(
        SAVED_WEATHER_DATA_REF,
        JSON.stringify(newWeatherData[+currentMenyKeyRef])
      );
      localStorage.setItem(
        CURRENT_WEATHER_DATA,
        JSON.stringify(newWeatherData[+currentMenyKeyRef])
  ...
  }, [allCitiesWeatherData, menuKeyRef, savedWeatherDataRef, dispatch]);

  const menuItems = useMemo(
    (): MenuItemType[] =>
      allCitiesWeatherData.map((item, index) => {
    ...
    [allCitiesWeatherData, handleDeleteBtnClick, isLoading]
  );
  ...
  const handleMenuItemSelect = useCallback(
    (menuInfo: MenuInfo): void => {
      ...
      dispatch(
        setCurrentWeatherData(allCitiesWeatherData[Number(menuInfo.key)])
      );
      savedWeatherDataRef.current = allCitiesWeatherData[Number(menuInfo.key)];
    },
    [allCitiesWeatherData, dispatch, menuKeyRef, savedWeatherDataRef]
  );
  ...
  useEffect(() => {
    ...
    if (menuItems.length)
      document.addEventListener("mouseover", handleMouseOverSider);

    return () =>
      document.removeEventListener("mouseover", handleMouseOverSider);
  }, [siderRef, menuItems, dispatch]);
