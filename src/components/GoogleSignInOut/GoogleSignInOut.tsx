...
import { logOutUser } from "../../helpers/requests/user/logOutUser/logOutUser";
...
const GoogleSignInOut: React.FC<Props> = ({
  ...
  const handleLogin = useGoogleLogin({
    ...
    onSuccess: (response) => {
      ...
      localStorage.setItem(
        LOCAL_STORAGE_ITEMS.USER_TOKEN,
  ...
  }, [userData, userToken, userError, handleLogin, dispatch]);
  ...
  useEffect(() => {
    const handleOutsideUserProfileClick = (e: MouseEvent): void => {
      ...
      if (profileActionsRef.current) {
        if (
          userProfileContainer?.contains(target) &&
    ...
    return () =>
      document.removeEventListener("mousedown", handleOutsideUserProfileClick);
  }, []);
    ...
    <>
      ...
      {userData && (
        <div className="user-profile-container">
          ...
          <p className="user-profile-actions" ref={profileActionsRef}>
